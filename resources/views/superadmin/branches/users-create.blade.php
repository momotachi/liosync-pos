...
@section('title', 'Add User - ' . $branch->name)
...
<div class="mb-6">
    <div class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 mb-2">
        ...
        <a href="{{ route('company.branches.users.index', [$company, $branch]) }}" class="hover:text-primary transition-colors">{{ $branch->name }} - Users</a>
        <span class="material-symbols-outlined text-sm">chevron_right</span>
        <span>Add User</span>
    </div>
    <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Add User to {{ $branch->name }}</h1>
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Create a new user for this branch</p>
    </div>
</div>

<!-- Form Card -->
<div class="bg-card-light dark:bg-card-dark rounded-xl shadow-sm border border-border-light dark:border-border-dark p-6">
    <form method="POST" action="{{ route('company.branches.users.store', [$company, $branch]) }}">
        @csrf
        ...
        <div class="flex items-center justify-end gap-3 pt-4 border-t border-border-light dark:border-border-dark">
            <a href="{{ route('company.branches.users.index', [$company, $branch]) }}"

...
@section('title', 'Sales - Superadmin')

@section('content')
<!-- Page Header -->
<div class="mb-6">
    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Sales Overview</h1>
    <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Sales data aggregated from all companies</p>
</div>
    ...
    <div class="bg-card-light dark:bg-card-dark rounded-xl shadow-sm border border-border-light dark:border-border-dark p-6">
        <div class="flex items-center justify-between">
            ...
            <div class="h-12 w-12 rounded-lg bg-green-50 dark:bg-green-900/30 flex items-center justify-center">
                <span class="material-symbols-outlined text-green-600 dark:text-green-400">attach_money</span>
            </div>
        </div>
    </div>
    ...
    <div class="bg-card-light dark:bg-card-dark rounded-xl shadow-sm border border-border-light dark:border-border-dark p-6">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-2xl font-bold text-gray-900 dark:text-white mt-1">
                    {{ $totalOrders > 0 ? number_format($totalSales / $totalOrders, 0, ',', '.') : '0' }}
                </p>
            </div>
        ...
        <table class="min-w-full divide-y divide-border-light dark:divide-border-dark">
            <thead class="bg-gray-50 dark:bg-gray-800">
                <tr>
                    ...
                    <th class="px-6 py-3 text-right text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Total Sales</th>
                ...
                @foreach($companies as $company)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                        ...
                        <td class="px-6 py-4 text-right text-sm font-medium text-gray-900 dark:text-white">
                            {{ number_format($company->total_sales ?? 0, 0, ',', '.') }}
                        </td>
                        <td class="px-6 py-4 text-right text-sm text-gray-600 dark:text-gray-400">
                            {{ $company->total_orders ?? 0 }}
                        </td>
                        <td class="px-6 py-4 text-right text-sm text-gray-600 dark:text-gray-400">
                            {{ ($company->total_orders ?? 0) > 0 ? number_format(($company->total_sales ?? 0) / ($company->total_orders ?? 1), 0, ',', '.') : '0' }}
                        </td>
